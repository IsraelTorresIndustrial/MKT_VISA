#!/usr/bin/env python3
"""
generate_email_maker_guides.py — Generador de Guías de Email Maker para LLMs
Genera:
...
bch_assets = [x for x in data if x.get('bank') == 'Banco de Chile']
visa_assets = [x for x in data if x.get('bank') == 'Visa']

# ==============================================================================
# 1. BCH HTML EMAIL GUIDE
# ==============================================================================
bch_3d = {x['filename']: x for x in bch_assets if x.get('category') == 'Íconos 3D'}
bch_2d = {x['filename']: x for x in bch_assets if x.get('category') == 'Íconos 2D'}
bch_travel = [x for x in bch_assets if x.get('category') == 'Travel & Turismo']
bch_logos = [x for x in bch_assets if x.get('category') == 'Logotipos']

bch_guide = """# ✉️ Banco de Chile · Guía Maestra para Generación de Emails HTML con IA & LLMs
...
# ==============================================================================
# 2. VISA HTML EMAIL GUIDE
# ==============================================================================
visa_guide = """# ✉️ Visa · Guía Maestra para Generación de Emails HTML con IA & LLMs
> **Versión:** 2.0 (Compatible con Email Maker Engine / Adobe AJO / SFMC / Mailchimp / HTML Headless)  
...
print(f"✅ Generado: {bch_out} ({len(bch_guide.splitlines())} líneas)")
print(f"✅ Generado: {visa_out} ({len(visa_guide.splitlines())} líneas)")
